...
router.get('/exportBackup', async (req, res) =>{
	...
	execute(`export PGPASSWORD=\'${password}\'; pg_dump -h ${host} -U ${username} -d ${database} -f ${fileName} -F t`,).then(async () => {
        console.log("Dumped");
    }).catch(err => {
        console.log(err);
    });
...
router.get('/importBackup', async (req, res) =>{
    ...
    execute(`export PGPASSWORD=\'${password}\'; pg_restore --data-only -h ${host} -U ${username} -d ${database} ${fileName}`).then(async () => {
...
router.get('/importSample', async (req, res) =>{
    ...
    await pool.query(sql).then(async () => {
